...
export function NavBar({ title, children }: Props) {
  return (
    ...
    >
      <Toolbar>
        <WikiPlantsLogoNoLoVeniasVenirLogo title={title} />
        {children}
      </Toolbar>
    </AppBar>
  )
...
const faviconOptionsLength = faviconsList.length

function WikiPlantsLogoNoLoVeniasVenirLogo({ title }: { title: string }) {
  ...
  const favicon = faviconsList[faviconIndex]
